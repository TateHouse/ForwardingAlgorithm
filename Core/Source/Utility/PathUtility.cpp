...
std::filesystem::path PathUtility::ValidatePath(const std::filesystem::path& path, const std::string_view extension)
{
	...
	{
		const auto message{std::format("The specified path does not exist. Path: {}", path.string())};
		throw std::invalid_argument{message};
	}

	if (!std::filesystem::is_regular_file(path))
	{
		const auto message{std::format("The specified path is not a regular file. Path: {}", path.string())};
		throw std::invalid_argument{message};
	}
